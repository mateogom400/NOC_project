...
│   ├── robot_real_lidar/       # ★ /utlidar/cloud → /lidar/points_filtered adapter
...
| [`robot_real_lidar/`](src/robot_real_lidar/) | Adapter for the built-in Unitree L1 LiDAR: `/utlidar/cloud` → `/lidar/points_filtered` (range + height filter, voxel downsample, TF to `odom`) |
...
| [`robot_real_goal_manager/`](src/robot_real_goal_manager/) | RViz `/goal_pose` → `/global_goal` relay + waypoint-mission runner (YAML-driven benchmarks) |
