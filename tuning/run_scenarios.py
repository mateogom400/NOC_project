#!/usr/bin/env python3
"""
Run a list of navigation scenarios by launching:

  ros2 launch robot_sim sim_a_star_mpc.launch.py

for each scenario, then recording a bag per scenario and restarting the full
simulation between runs.

This script is intentionally close to the launch flow used by
`bayesian_mpc_tuner.py`, but without Bayesian optimization.

Example:
  python3 tuning/run_scenarios.py --world warehouse.world --world-pkg sim_worlds
  python3 tuning/run_scenarios.py --world default.sdf --world-pkg go2_sim --gui
  python3 tuning/run_scenarios.py --names open_square,warehouse_loop
"""
...
def main():
    ap = argparse.ArgumentParser(description="Run navigation scenarios with one launch per scenario")
    ap.add_argument(
        "--tuner",
        type=str,
        default=str(Path(__file__).parent / "bayesian_mpc_tuner.py"),
        help="Path to bayesian_mpc_tuner.py to reuse helpers and scenario definitions",
    )
    ap.add_argument(
        "--names",
        type=str,
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Run only scenarios that use this world (e.g. warehouse.world)",
    )
    ap.add_argument(
        "--world-pkg",
        type=str,
        default=None,
        help="Optional package filter together with --world (e.g. sim_worlds)",
    )
    ap.add_argument(
        "--outdir",
        type=str,
        default=None,
        help="Output directory for scenario bags (default chosen from world)",
    )
    ap.add_argument("--gui", action="store_true", help="Launch Gazebo and RViz GUI")
    ap.add_argument(
        "--duration",
        type=int,
        default=None,
        help="Maximum duration per scenario in seconds",
    )
    ap.add_argument(
        "--no-cleanup",
        action="store_true",
        help="Skip extra cleanup delay between scenarios",
    )
    args = ap.parse_args()
    ...
    try:
        for sc in run_scenarios:
            ...
            try:
                ...
                # Launch the exact file requested by the user via SimulationManager.
